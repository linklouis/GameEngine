...
public final class LayerCollider2D extends Collider2D<LayerCollider2D> {
    /**
    ...
    public LayerCollider2D() {
        super();
    }
    public LayerCollider2D(Modifier... modifiers) {
        super(modifiers);
    }
...
//    }
    ...
    @Override
    public boolean isColliding(LayerCollider2D coll) {
...
//        double thisMinX = minX();
    ...
    public Row getColliding(GameObject gObj) { // TODO
        assert gObj.containsModifier(LayerCollider2D.class);
        LayerCollider2D coll = gObj.get(LayerCollider2D.class);
        if (inRange(coll)) {
            for (double y = Math.max(minY(), coll.minY()); y < Math.min(maxY(), coll.maxY()); y += ROW_HEIGHT) {
                if (rowAt(y).atX(getX()).isColliding(coll.rowAt(y).atX(coll.getX()))) {
                    return coll.rowAt(y);
    ...
    @Override
    public Class<LayerCollider2D> getColliderClass() {
        return LayerCollider2D.class;
    }
    ...
    public Row rowAt(double y) {
        if (y > maxY() || y < minY()) {
            throw new IllegalArgumentException("y value of "
                    + y + " out of bounds for gameengine.skeletons.GameObject of height "
    ...
    public static class Row extends Collider2D<Row> {
