...
public class FillDark  implements PostProcess {
...
//                Vector2D pixelLoc = new Vector2D(x, y);
//
//                Vector3D totalWeighted = new Vector3D();
//                double totalWeight = 0;
//                for (int x1 = x - RANGE; x1 < x + RANGE; x1++) {
//                    for (int y1 = y - RANGE; y1 < y + RANGE; y1++) {
//                        int x2 = (int) clamp(x1, 0, image.getWidth() - 1);
//                        int y2 = (int) clamp(y1, 0, image.getHeight() - 1);
//                        if (collisionMap[x][y].equals(collisionMap[x2][y2]) && pixelLoc.distance(new Vector2D(x2, y2)) < RANGE) {
//                            double dist = pixelLoc.distance(new Vector2D(x2, y2));
//                            totalWeighted = totalWeighted.add(new Vector3D(reader.getColor(x2, y2)).scalarMultiply(dist));
//                            totalWeight += dist;
//                        }
//                    }
//                }
//
//                Vector3D averageColor = totalWeighted.scalarDivide(totalWeight);
//
//                if (averageColor.magnitude() >= MIN_BRIGHTNESS &&
//                        averageColor.distance(color) > ACCEPTABLE_DIFF) {
//                    buffer.put((int) (x + y * image.getWidth()), averageColor.oneInt());
//                } else {
//                    buffer.put((int) (x + y * image.getWidth()), new Vector3D(reader.getColor(x, y)).oneInt());
...
//    }
    ...
    private WritableImage processOnea(WritableImage image, RayTraceable[][] collisionMap) {
        PixelReader reader = image.getPixelReader();
        IntBuffer buffer = IntBuffer.allocate((int) (image.getWidth() * image.getHeight() * Integer.BYTES));
